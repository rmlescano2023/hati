/**
 * Dev-only harness: renders the eSOA to /tmp with a large demo data set so the
 * pagination behaviour can be inspected without hand-entering data.
 * Run with:  npx vite-node scripts/render-esoa-check.tsx
 */
import { createElement } from 'react';
import { Font, renderToFile } from '@react-pdf/renderer';
import { resolve } from 'node:path';
import { EsoaDocument } from '../src/pdf/EsoaDocument';
...
const out = process.argv[4] ?? '/tmp/hati-esoa-check.pdf';

const data = buildDemoData(memberCount, itemCount);

await renderToFile(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  createElement(EsoaDocument, { members: data.members, records: data.records }) as any,
  out,
);
