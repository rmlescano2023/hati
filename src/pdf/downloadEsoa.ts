...
/**
 * Build the eSOA and hand it to the browser as a real file download.
 *
 ...
 * initial page bundle.
 */
export async function downloadEsoa({ members, records }: Args): Promise<void> {
  const [{ pdf }, { registerPdfFonts }, { EsoaDocument }] = await Promise.all([
  ...
  const element = createElement(EsoaDocument, {
    members,
    records,
